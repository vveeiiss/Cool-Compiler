    ...
    , error_stream(cerr)
{
    ...
    for (int i = classes->first(); classes->more(i); i = classes->next(i)) {
        ...
        if (m_classes.find(classes->nth(i)->GetName()) == m_classes.end()) {
            ...
            semant_error(classes->nth(i))
                << "Error! Class " << classes->nth(i)->GetName() << " has been defined!"
                << std::endl;
            return;
        }
    }
    ...
    for (int i = classes->first(); classes->more(i); i = classes->next(i)) {
        ...
        while (parent_name != Object && parent_name != classes->nth(i)->GetName()) {
            ...
            if (parent_name == Int || parent_name == Str || parent_name == SELF_TYPE || parent_name == Bool) {
                semant_error(curr_class)
                    << "Error! Class " << curr_class->GetName() << " cannot inherit from "
...
std::list<Symbol>
ClassTable::GetInheritancePath(Symbol type)
{
    if (type == SELF_TYPE) {
...
void method_class::AddMethodToTable(Symbol class_name)
{
    ...
    methodtables[class_name].addid(name,
        new method_class(copy_Symbol(name),
            formals->copy_list(),
...
void attr_class::AddAttribToTable(Symbol class_name)
{
    ...
    if (name == self) {
        classtable->semant_error(curr_class)
            ...
            << curr_class->GetName() << std::endl;
    }
    if (attribtable.lookup(name) != NULL) {
        classtable->semant_error(curr_class)
            << "Error! attribute '" << name << "' already exists!" << std::endl;
        return;
    }
...
Symbol
static_dispatch_class::CheckExprType()
{
    bool error = false;
...
//
Symbol
cond_class::CheckExprType()
{
    if (pred->CheckExprType() != Bool) {
        classtable->semant_error(curr_class)
            << "Error! Type of pred is not Bool." << std::endl;
    }
...
//
Symbol
let_class::CheckExprType()
{
    if (identifier == self) {
        classtable->semant_error(curr_class)
            << "Error! self in let binding." << std::endl;
    }
    ...
    if (init_type != No_type) {
        if (classtable->IsParent(type_decl, init_type) == false) {
            classtable->semant_error(curr_class)
                << "Error! init value is not child." << std::endl;
        }
    }
...
eq_class::CheckExprType()
{
    ...
    if (e1_type == Int || e2_type == Int || e1_type == Bool || e2_type == Bool || e1_type == Str || e2_type == Str) {
        if (e1_type != e2_type) {
            classtable->semant_error(curr_class)
                << "Error! '=' meets different types." << std::endl;
            type = Object;
        } else {
            type = Bool;
        }
...
Symbol
comp_class::CheckExprType()
{
    if (e1->CheckExprType() != Bool) {
        classtable->semant_error(curr_class)
            << "Error! 'not' meets non-Bool value." << std::endl;
        type = Object;
    } else {
        type = Bool;
    }
...
Symbol
bool_const_class::CheckExprType()
{
    type = Bool;
    return type;
}
...
Symbol
string_const_class::CheckExprType()
{
    type = Str;
    return type;
}
...
Symbol
no_expr_class::CheckExprType()
{
    return No_type;
}

Symbol
object_class::CheckExprType()
{
    if (name == self) {
        type = SELF_TYPE;
        return type;
    ...
    if (found_type == NULL) {
        classtable->semant_error(curr_class)
            << "Cannot find object " << name << std::endl;
        type = Object;
    } else {
        type = *found_type;
    }
                    ...
                    for (; formals->more(k1) && curr_formals->more(k2);
                        ...
                        if (formals->nth(k1)->GetType() != curr_formals->nth(k2)->GetType()) {
                            ...
                            classtable->semant_error(classtable->m_classes[class_name])
                                << "Method override error: formal type not match." << std::endl;
                        }
                    }
